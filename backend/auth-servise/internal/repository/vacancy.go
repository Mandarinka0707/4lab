...
func (r *VacancyRepository) AdminUpdate(ctx context.Context, vacancy *entity.Vacancy) error {
	fmt.Printf("Starting admin vacancy update in repository for ID: %d\n", vacancy.ID)
	query := `
		UPDATE vacancies 
		SET title = $1, description = $2, requirements = $3, responsibilities = $4,
			salary = $5, location = $6, employment_type = $7, company = $8,
	...
	fmt.Printf("Executing admin update query with values: %+v\n", vacancy)
	err := r.db.QueryRowContext(
		ctx,
		query,
		vacancy.Title,
